// Tideland Go Library - DSA - Collections - Sets - Unit Tests
//
...
func TestSetsDoAll(t *testing.T) {
	...
	assert.ErrorMatch(err, ".* cannot process all entries: ouch")
}
...
func TestStringSetsFindAll(t *testing.T) {
	...
	assert.ErrorMatch(err, ".* cannot find all matching string values: ouch")
	assert.Length(vs, 0)
}
...
func TestStringSetsDoAll(t *testing.T) {
	...
	assert.ErrorMatch(err, ".* cannot process all string entries: ouch")
}
