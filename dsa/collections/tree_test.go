/// Tideland Go Library - DSA - Collections - Trees - Unit Tests
//
...
func TestTreeCreate(t *testing.T) {
	...
	assert.ErrorMatch(err, ".* cannot find node")
	...
	assert.Nil(err)
	err = tree.At("root").Add("bravo")
	assert.ErrorMatch(err, ".* adding duplicate node is not allowed")
}
...
func TestTreeSetValue(t *testing.T) {
	...
	old, err = tree.At("root", "alpha").SetValue("beta")
	assert.Nil(old)
	assert.ErrorMatch(err, ".* setting duplicate value is not allowed")
}
...
func TestTreeFind(t *testing.T) {
	...
	assert.ErrorMatch(err, ".* cannot find first matching node")
	_, err = tree.FindFirst(func(v interface{}) (bool, error) {
		return false, errors.New("ouch")
	}).List()
	assert.ErrorMatch(err, ".* cannot find first matching node: ouch")
...
func TestStringTreeSetValue(t *testing.T) {
	...
	old, err = tree.At("root", "alpha").SetValue("beta")
	assert.Equal(old, "")
	assert.ErrorMatch(err, ".* setting duplicate string value is not allowed")
}
...
func TestKeyValueTreeSetKey(t *testing.T) {
	...
	assert.ErrorMatch(err, ".* setting duplicate key is not allowed")
}
...
func createTree(assert *asserts.Asserts) *collections.Tree {
	tree := collections.NewTree("root", true)
	err := tree.At("root").Add("alpha")
	assert.Nil(err)
	err = tree.At("root").Add("bravo")
...
func createStringTree(assert *asserts.Asserts) *collections.StringTree {
	tree := collections.NewStringTree("root", true)
	err := tree.At("root").Add("alpha")
	assert.Nil(err)
	err = tree.At("root").Add("bravo")
...
func createKeyValueTree(assert *asserts.Asserts) *collections.KeyValueTree {
	tree := collections.NewKeyValueTree("root", 1, true)
	err := tree.At("root").Add("alpha", 2)
	assert.Nil(err)
	err = tree.At("root").Add("bravo", 3)
...
func createKeyStringValueTree(assert *asserts.Asserts) *collections.KeyStringValueTree {
	tree := collections.NewKeyStringValueTree("root", "one", true)
	err := tree.At("root").Add("alpha", "two")
	assert.Nil(err)
	err = tree.At("root").Add("bravo", "three")
