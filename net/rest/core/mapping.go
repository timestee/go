...
func (hl *handlerList) register(handler ResourceHandler) error {
	...
	for {
		if current.handler == handler {
			return errors.New(ErrDuplicateHandler, msgDuplicateHandler, handler.ID())
		}
		if current.next == nil {
			break
		}
...
func (m *mapping) handle(job *Job) error {
	...
	if m.ignoreFavicon {
		if job.Path().Domain() == "favicon.ico" {
...
func (m *mapping) handlerList(job *Job) (*handlerList, error) {
	location := m.location(job.Path().Domain(), job.Path().Resource())
	hl, ok := m.handlers[location]
	if ok {
		return hl, nil
	}
	location = m.location(job.Path().Domain(), job.Environment().DefaultResource())
	hl, ok = m.handlers[location]
	if ok {
		return hl, nil
	}
	...
	return nil, errors.New(ErrNoHandler, msgNoHandler, location)
}
