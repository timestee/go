...
func (gf *gobFormatter) Read(data interface{}) error {
	if !gf.job.HasContentType(ContentTypeGOB) {
		return errors.New(ErrInvalidContentType, msgInvalidContentType, ContentTypeGOB)
	}
...
func (jf *jsonFormatter) Read(data interface{}) error {
	if !jf.job.HasContentType(ContentTypeJSON) {
		return errors.New(ErrInvalidContentType, msgInvalidContentType, ContentTypeJSON)
	}
...
func (xf *xmlFormatter) Read(data interface{}) error {
	if !xf.job.HasContentType(ContentTypeXML) {
		return errors.New(ErrInvalidContentType, msgInvalidContentType, ContentTypeXML)
	}
...
func (qv queryValuer) Value() (string, error) {
	v := string(qv)
	if len(v) == 0 {
		return "", errors.New(ErrQueryValueNotFound, msgQueryValueNotFound)
	}
	return v, nil
}
