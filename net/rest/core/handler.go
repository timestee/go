...
func handleJob(handler ResourceHandler, job *Job) (bool, error) {
	...
	return false, errors.New(ErrMethodNotSupported, msgMethodNotSupported, job.Request().Method)
}
...
func handleGetJob(handler ResourceHandler, job *Job) (bool, error) {
	...
	return false, errors.New(ErrMethodNotSupported, msgMethodNotSupported, jobDescription(handler, job))
}
...
func jobDescription(handler ResourceHandler, job *Job) string {
	return fmt.Sprintf("%s %s@%s/%s", job.Request().Method, handler.ID(), job.Path().Domain(), job.Path().Resource())
}
